/*
 * config.c - Configuration management for solpkg
 *
...
int config_init_dirs(void)
{
    ...
    mkdirs(SOLPKG_VAR);
    mkdirs(SOLPKG_CACHE);
    mkdirs(SOLPKG_ROLLBACK);
    mkdirs(SOLPKG_IDX_DIR);
    return 0;
}
...
void config_defaults(solpkg_config_t *cfg)
{
    ...
    strcpy(cfg->ca_bundle, SOLPKG_CA_BUNDLE);
...
int config_save(const solpkg_config_t *cfg)
{
    ...
    f = fopen(SOLPKG_CONF, "w");
    if (!f) {
        fprintf(stderr, "solpkg: cannot write %s: %s\n",
                SOLPKG_CONF, strerror(errno));
        return -1;
    }

    fprintf(f, "# solpkg repository configuration\n");
    fprintf(f, "# Generated by solpkg %s\n\n", SOLPKG_VERSION);
